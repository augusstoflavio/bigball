...
import com.augusto.bigball.presentation.login.formstate.SignupFormState
...
class SignupViewModel(
    ...
    fun signup(name: String, username: String, password: String, passwordConfirmation: String) {
        if (!validInputs(name, username, password)) {
            return
        }

        run {
            when (val response = signupUseCase(name, username, password, passwordConfirmation)) {
                is Result.Success -> {
