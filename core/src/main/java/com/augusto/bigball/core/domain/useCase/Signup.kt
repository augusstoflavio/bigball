...
class Signup(private val authRepository: AuthRepository) {
    ...
    suspend operator fun invoke(name: String, email: String, password: String, passwordConfirmation: String): Result<Boolean> {
        return authRepository.signup(name, email, password, passwordConfirmation)
    }
}